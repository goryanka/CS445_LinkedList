...
public class LinkedListTester
{
	...
	{
		LinkedList list1 = new LinkedList( args[0], false );  // false = INSERTATTAIL CALLED ON EACH T READ FROM FILE
		...
		LinkedList list2 = new LinkedList( args[0], true );  // true = INSERTINORDER CALLED ON EACH T READ FROM FILE
		System.out.format("list2 %s size %d ordered %s\n",args[0],list2.size(),list2 ); // invokes toString

		LinkedList list3 = new LinkedList( args[1], true );  // true = INSERTINORDER CALLED ON EACH T READ FROM FILE
		...
		LinkedList unionList = list2.union(list3);
		System.out.println("list2.union(list3) " + unionList); 	
		
		LinkedList interList = list2.inter(list3);	
		System.out.println("list2.inter(list3) " + interList); 
		
		LinkedList diffList = list2.diff(list3);		
		System.out.println("list2.diff(list3)  " + diffList); 
		
		LinkedList xorList = list2.xor(list3);				
